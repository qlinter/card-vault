...
本次新增收藏状态调整：持有、待送评、送评中、待售、已售，移除目标卡状态。前四项统一计入持仓、估值和覆盖率，已售保留历史现金流与已实现盈亏；状态编辑校验剩余数量。组合汇总、分布和排行共用预计算的数量与报价，覆盖分母统一读取。规则见[财务历史与持仓模型](./docs/financial-history-model.md)。
