...
import type { PortfolioAnalysis, PortfolioFilterInput, PortfolioScope, PortfolioSnapshot } from "@/lib/portfolio-analysis";
import { errorMessage } from "@/lib/feedback-messages";
import styles from "./portfolio-analysis.module.css";

type AnalysisResponse = { analysis?: PortfolioAnalysis; snapshot?: PortfolioSnapshot; provider?: string; error?: string };
...
export function PortfolioAnalysisButton({ cardCount, query, scope }: PortfolioAnalysisButtonProps) {
  ...
  useEffect(() => { setAnalysis(null); setSnapshot(null); setError(null); setOpen(false); }, [fingerprint]);
  useEffect(() => {
  ...
  async function analyze() {
    ...
    try {
      ...
      setAnalysis(data.analysis); setSnapshot(data.snapshot); setProvider(data.provider ?? "统一 AI");
    } catch (requestError) {
