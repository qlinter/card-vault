...
项目按职责维护：`app` 负责路由及操作入口，`components` 负责界面，`lib` 负责领域规则和服务，`electron` 负责桌面、存储与 IPC，`scripts` 负责初始化、验收和发布，`tests` 保留回归用例及视觉基线。录入与导入的初始财务写入统一由 `lib/card-entry-finance-service.ts` 编排，卡片图片处理保留在卡片录入服务；表单解析与财务核算各自共享，不复制整套页面流程。
