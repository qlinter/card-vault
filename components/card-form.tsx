    ...
    <form ref={formRef} action={action} onSubmit={onSubmit} onInvalid={onInvalid} className="panel" encType="multipart/form-data" data-card-entry-form={mode === "create" ? "true" : undefined}>
      ...
      <div className="form-grid">
        ...
        <label className="field full">
          ...
          <small className="muted"><UiText text={"填写“编号”或“编号范围”时会自动判定为限量卡；也可以在尚未补全编号时手动勾选。"} /></small>
        </label>
        ...
        {mode === "create" ? (
          <>
            <div className="form-section-heading full">
              <div>
                <h2><UiText text={"初始财务记录"} /></h2>
                <p><UiText text={"这些内容会分别保存为购买交易、评级费用和估值记录，之后请在卡片详情页继续维护。"} /></p>
              </div>
            </div>
        ...
        <label className="field full">
          ...
          <UiElement as="textarea" uiAttributes={["placeholder"]}
        ...
        <label className="field">
          <span>
            <input name="isRookie" type="checkbox" defaultChecked={values?.isRookie ?? card?.isRookie ?? false} /><UiText text={"是否 Rookie"} /></span>
        </label>

        <label className="field">
          <span>
            <input name="isAutograph" type="checkbox" defaultChecked={values?.isAutograph ?? card?.isAutograph ?? false} /><UiText text={"是否签名卡"} /></span>
        </label>

        <label className="field">
          <span><UiText text={"签字类型"} /></span>
          <UiElement as="input" uiAttributes={["placeholder"]} name="autoType" placeholder="例如 on-card / sticker" defaultValue={pickValue(values?.autoType, card?.autoType ?? "")} />
        </label>

        <label className="field">
          <span>
            <input name="isPatch" type="checkbox" defaultChecked={values?.isPatch ?? card?.isPatch ?? false} /><UiText text={"是否 Patch/Jersey"} /></span>
        </label>

        <label className="field">
          <span><UiText text={"Patch 类型"} /></span>
          <UiElement as="input" uiAttributes={["placeholder"]} name="patchType" placeholder="例如 multi-color / logo patch" defaultValue={pickValue(values?.patchType, card?.patchType ?? "")} />
        </label>
      ...
      <div className="card-form-actions">
        {mode === "create" ? (
          <>
            ...
            <button disabled={submitDisabled} type="submit" name="saveIntent" value="copy" className="btn btn-secondary"><UiText text={"保存并复制新增"} /></button>
          </>
        ) : (
        ...
        {mode === "create" ? <small className="entry-keyboard-hint"><UiText text={"Ctrl + Enter 保存并继续 · Ctrl + Shift + Enter 保存并查看"} /></small> : null}
      </div>
    </form>
  );
}
