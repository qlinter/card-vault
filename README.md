...
源码版本：`1.3.4`（2026-09-19 小更新），汇总录入分区、补充财务记录、首页估值显隐、心愿删除、筛选组合导航与代码整理。范围和验证记录见 [v1.3.4 发布说明](./docs/release-v1.3.4.md)。经用户确认，已沿用 v1.3.4 版本号重新生成安装包和便携包。

Windows x64 未签名安装包 `dist/card-vault-1.3.4-setup.exe`、便携包 `dist/card-vault-1.3.4-portable.zip` 及 `dist/SHA256SUMS.txt` 已更新为 2026-09-19 小更新产物，包内运行与产物校验通过。2026-09-18 旧批次原样归档至 `backups/releases/v1.3.4-2026-09-18`；v1.3.3 分发文件保留在 `backups/releases/v1.3.3`。

## 最新版本：v1.3.4

本次小更新恢复首页筛选栏的组合入口与返回结果入口，链接使用无箭头、无下划线的文字样式；导航顺序为首页、组合、展示、分享、计划、设置。移除筛选生效提示、售出复盘空状态说明和视图区“全部收藏”，合并链接参数拼接并同步中英文说明。

1. **录入布局**：按卡片信息、评级信息、财务记录分区，宽窗口四列、窄窗口双列，统一控件高度，描述与备注并排；四项属性对齐，编号、签字类型和 Patch 类型自动关联勾选。
2. **新增记录**：保留原有快捷财务输入，在“财务记录”下补充多笔交易、费用或估值，随草稿保存、随卡片原子提交；详情页原流程保留。
3. **首页**：排序框默认显示“排序”，保留“最新录入”规则；栏目简化为“卡片”和“估值”，仅保留组合分析入口，眼睛图标切换并记忆估值显隐。
4. **界面精简**：移除保存并复制新增；说明集中到使用说明；新建分享和组合去除冗余文案，导出 CSV/XLSX 与前方控件对齐，视图切换沿用首页样式。
5. **心愿单**：移除取消心愿和取消编辑，编辑区提供删除心愿，确认后刷新列表和预算；历史已取消的心愿保留。
6. **维护性**：录入和导入共用初始财务服务，复用费用选项，移除闲置分支与重复样式；修复更换 AI 图片后旧属性残留，加强财务载荷与客户端边界检查，收敛重复文档。

数据库结构、既有财务核算规则及备份格式保持不变。本次小更新的包内验证已通过，Windows 实际安装升级验证继续暂缓。
...
以下保留全部 28 个历史版本，按新到旧排列；摘要描述各版本发布时的变化，当前行为与兼容范围以最新规范为准。中英文摘要从 v1.2.1 的历史 README 恢复；早期未找到独立发布说明，出处与缺失情况见[版本历史资料说明](./docs/version-history-sources.md)。
...
- [v1.3.4 发布说明](./docs/release-v1.3.4.md)：录入、财务补充、估值显隐、验收与分发状态。
