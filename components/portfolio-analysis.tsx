  ...
  return <>
    ...
    {mounted && open ? createPortal(<div className={styles.backdrop} onMouseDown={(event) => { if (event.target === event.currentTarget) setOpen(false); }} role="presentation">
      <section className={styles.modal} role="dialog" aria-modal="true" aria-busy={loading} aria-labelledby="portfolio-analysis-title">
        <header className={styles.modalHeader}><div><h2 id="portfolio-analysis-title">组合分析</h2><p>基于当前首页筛选结果中的 {displayCardCount} 张卡片</p></div><button type="button" className={styles.close} onClick={() => setOpen(false)} aria-label="关闭组合分析">×</button></header>
        ...
        <FinancialReportNote accounting={snapshot.accounting} showBasis={false} showRates={false} /><section className={styles.financialOverview} aria-label="财务摘要">{snapshot.financials.currencies.map((item) => <article key={item.currency}><header><strong>{item.currency}</strong><span>{item.valuedCardCount} 张有最新估值</span></header><dl><div><dt>持仓估值</dt><dd>{currency(item.latestValue, item.currency)}</dd></div><div><dt>剩余成本</dt><dd>{currency(item.activeCostBasis, item.currency)}</dd></div><div><dt>已实现盈亏</dt><dd>{currency(item.realizedProfit, item.currency)}</dd></div><div><dt>未实现盈亏</dt><dd>{currency(item.unrealizedDifference, item.currency)}</dd></div></dl></article>)}</section>
