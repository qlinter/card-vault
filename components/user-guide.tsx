...
const sections = [
  ...
  { id: "data", zh: "数据", en: "Data", items: [
    ...
    ["导出支持搜索和列表/表格切换，右侧分段按钮选择视图。点击卡片信息进入详情，返回上一页会恢复数据展开状态、搜索、页码、视图和勾选。CSV/XLSX 导出按当前搜索范围，不按勾选范围；私密 XLSX 另含财务流水，仅公开导出排除私人备注和财务流水。图片及完整恢复请使用备份。", "Export supports search and list/table views, selected with the segmented control on the right. Open card details from the card information. Going back restores the expanded Data section, search, page, view and selection. CSV/XLSX exports use the search scope, not checked rows. Private XLSX includes financial history; public-only exports exclude private notes and financial history. Use backups for images and full restoration."]
  ] },
  { id: "plans", zh: "计划", en: "Plans", items: [
    ...
    ["缺图、缺购买成本和缺估值会进入待整理。估值、送评和在售均满 180 天提醒：估值从最近估值日计算，送评和在售从最近档案修改日计算；已售和目标卡不产生持仓估值提醒。", "Missing images, purchase costs or valuations enter the review queue. Valuation, grading and listing reminders all start at 180 days. Valuations age from the latest valuation date; grading and listings age from the last archive edit. Sold and target cards do not receive holding valuation reminders."],
    ["每项可完成、忽略、延后 7 天或重新打开；勾选“显示已处理”可查看已处理项目。条件消失后提醒移除，相关资料变化后可能重新出现。摘要和提醒只在计划页查看，不发送系统通知。", "Mark a reminder done, dismiss it, snooze it for 7 days or reopen it. Show handled reminders includes handled items. Resolved conditions leave the queue; changed evidence can reopen reminders. Digests and reminders are viewed in Plans only; no system notifications are sent."],
  ...
  { id: "settings", zh: "设置、备份与 AI", en: "Settings, backups & AI", items: [
    ...
    ["完整备份保存数据库、图片、分享、批次和计划状态；CSV/XLSX 不能替代备份。恢复前核对来源并保留当前数据，系统验证媒体和校验清单、保留安全备份并生成迁移报告。", "Full backups preserve the database, images, shares, batches and plan state. CSV/XLSX files cannot replace backups. Verify the source and preserve current data before restoring. The app checks media and manifests, retains a safety backup and creates a migration report."],
