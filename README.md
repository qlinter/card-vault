...
源码版本：`1.3.2`（2026-09-09），安全、可靠性与大收藏性能更新。审核范围、改进及验证结果统一见 [v1.3.2 发布说明](./docs/release-v1.3.2.md)。

已于 2026-09-09 生成 Windows x64 未签名安装包 `dist/card-vault-1.3.2-setup.exe`、便携包 `dist/card-vault-1.3.2-portable.zip` 和 `dist/SHA256SUMS.txt`。完整发布检查、包内运行、版本、分发内容和 SHA-256 验证通过。

## 最新版本：v1.3.2

1. **依赖安全**：Next.js 更新至 16.3.3、sharp 至 0.35.4、js-yaml 至 4.3.2，保留全量依赖审计与原有失败门槛。
2. **启动诊断**：显示准备失败的底层错误和实际日志位置，完整读取中文错误输出。
3. **数据保护**：AI 和存储配置先完整写入临时文件再替换；恢复前再次核验复制副本的文件清单，阻止损坏媒体进入当前目录。
4. **路径与摘要修复**：正确识别 Windows 路径大小写和以双点开头的子目录；收藏摘要排除未来日期，兼容 SQLite 的文本和数字日期。
5. **代码优化**：计划提醒按 250 张分批处理，购买缺失信息只查存在性，摘要由数据库计数，最近估值改为单次扫描；分享分组不反复复制数组。
6. **组合历史**：按时间推进历史记录，在交易和费用不变时复用成本结果；每月金额、缺失标记与覆盖率保持一致。
7. **大规模导出**：CSV/XLSX 使用一致数据库快照、分批读取和磁盘流式写入，完成后分块下载；支持取消清理与超大工作表自动分表。
8. **代码收敛**：移除失效的旧查询及辅助函数，分享 ZIP 与 XLSX 共用流式归档；合并性能采样，收窄内部函数导出，并排除分发包中的开发检查和基准脚本。

财务核算、数据库结构和当前格式校验规则保持不变，不增加自动历史数据升级或调整页面布局。历史版本及分发事实保留在各自发布说明中。
...
以下保留全部 26 个历史版本，按新到旧排列；摘要描述各版本发布时的变化，当前行为与兼容范围以最新规范为准。中英文摘要从 v1.2.1 的历史 README 恢复；早期未找到独立发布说明，出处与缺失情况见[版本历史资料说明](./docs/version-history-sources.md)。
...
- [v1.3.2 发布说明](./docs/release-v1.3.2.md)：修复、代码优化、验收与分发状态。
