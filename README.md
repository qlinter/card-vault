...
源码版本：`1.3.4`（2026-09-18），汇总录入分区、补充财务记录、首页估值显隐、心愿删除与代码整理。范围和验证记录见 [v1.3.4 发布说明](./docs/release-v1.3.4.md)。

已生成 Windows x64 未签名安装包 `dist/card-vault-1.3.4-setup.exe`、便携包 `dist/card-vault-1.3.4-portable.zip` 及 `dist/SHA256SUMS.txt`，包内运行与产物校验通过。旧版 v1.3.3 分发文件已原样归档至 `backups/releases/v1.3.3`。
...
数据库结构、既有财务核算规则及备份格式保持不变。包内验证已通过，Windows 实际安装升级验证继续暂缓。
