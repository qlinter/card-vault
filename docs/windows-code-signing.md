...
手动运行 `.github/workflows/release-windows.yml` 可在 GitHub Windows Runner 上生成 Windows 产物。不配置签名 Secrets 时生成未签名产物；需要签名时任选一种方式配置仓库 Secrets：
...
```powershell
Get-AuthenticodeSignature -LiteralPath '.\dist\card-vault-1.2.1-setup.exe' |
