...
const sections = [
  ...
  { id: "cards", zh: "卡片录入与维护", en: "Card entry & maintenance", items: [
    ["保存并查看会打开新卡详情，保存并继续会进入下一次录入；快捷键分别为 Ctrl + Shift + Enter、Ctrl + Enter。草稿列表保留最近 8 项。", "Save and View opens the new card; Save and Continue starts the next entry. Shortcuts are Ctrl + Shift + Enter and Ctrl + Enter respectively. The draft list shows the eight most recent items."],
    ["填写编号或编号范围后，限量编号卡会自动勾选；没有编号时也可手动勾选。清空两项后，可手动取消。限量编号、签名、Patch/Jersey、Rookie 在评级字段上方同排，分别对齐四列。", "Entering a serial number or range automatically checks Numbered card; you can also check it manually without a number. Clear both fields before unchecking. Numbered, Autographed, Patch/Jersey and Rookie align in four columns above grading fields."],
    ["录入字段按卡片信息、评级信息、财务记录分区。签字类型和 Patch 类型位于属性选项上方；填写后自动勾选对应属性，清空类型后可手动取消。AI 填入与草稿恢复同样适用。", "Entry fields are grouped into Card Information, Grading Information and Financial Records. Autograph and Patch types appear above the attribute options; entering a type checks its attribute automatically. Clear the type before unchecking. This also applies to AI suggestions and restored drafts."],
    ["AI 可使用 1–2 张正反面图片，默认只填空字段，勾选覆盖后可更新已有内容；识别结果需核对，备注保持为空。展示描述用于展示和分享的单卡详情。", "AI accepts one or two front/back images and fills empty fields by default. Enable overwrite to update existing values; review suggestions before saving. Notes remain empty. Public descriptions appear in Showcase and shared card details."],
    ["初始财务内容分别保存为购买、评级费用和估值记录，后续在详情页维护。两种币种的付款属于同一次购入，数量只计一次；金额均留空表示成本未知，赠品填写 0。", "Initial financial values create purchase, grading expense and valuation records, maintained later in card details. Payments in two currencies belong to one purchase and count quantity once. Leave both amounts blank for unknown cost; enter 0 for a gift."],
    ["财务记录保留快捷输入；点击新增记录可补充交易、费用或估值，支持多笔并随草稿恢复。补充记录与卡片一同保存，出售费用需选择关联出售记录；同一笔业务不要在快捷字段和补充记录中重复填写。", "Keep quick financial inputs and use Add Record for additional transactions, expenses or valuations. Multiple records survive draft recovery and save with the card. Link sale expenses to a sale; do not enter the same activity in both quick fields and additional records."],
  ...
  { id: "data", zh: "数据", en: "Data", items: [
    ...
    ["导出支持搜索和列表/表格切换，右侧分段按钮选择视图。点击卡片信息进入详情，返回上一页会恢复数据展开状态、搜索、页码、视图和勾选。CSV/XLSX 有勾选时仅导出搜索范围内选中的卡片，无勾选时导出当前搜索结果；私密 XLSX 另含财务流水，仅公开导出排除私人备注和财务流水。图片及完整恢复请使用备份。大批导出先生成文件再开始下载，请等待完成；最多同时进行两个导出，生成限时 10 分钟。XLSX 超出单表行数时自动分表；导入仍受原有大小和行数限制。", "Export supports search and list/table views, selected with the segmented control on the right. Open card details from the card information. Going back restores the expanded Data section, search, page, view and selection. CSV/XLSX exports include checked cards within the search scope, or all search results when no cards are checked. Private XLSX includes financial history; public-only exports exclude private notes and financial history. Use backups for images and full restoration. Large exports finish generating before download starts; allow time to complete. Up to two exports can run at once, with a 10-minute generation limit. XLSX automatically splits oversized sheets; import size and row limits still apply."]
  ] },
  { id: "plans", zh: "计划", en: "Plans", items: [
    ...
    ["“新增”和“列表”可分别展开或收起。心愿可填写名称、主体、预算、币种、目标日期和备注；编辑已有心愿时自动展开输入区，可保存或删除心愿，删除前会确认。待实现预算按币种独立汇总；已实现和历史已取消的心愿不计入，可重新计划。", "New and List expand or collapse independently. Wishes include a title, subject, budget, currency, target date and notes. Editing opens the input section, where you can save or delete the wish with confirmation. Planned budgets are totaled separately by currency. Acquired and previously cancelled wishes are excluded and can be planned again."]
  ] },
  { id: "settings", zh: "设置、备份与 AI", en: "Settings, backups & AI", items: [
    ...
    ["DeepSeek：在 AI 中选择 DeepSeek，默认 Endpoint 为 https://api.deepseek.com/chat/completions，Model 为 deepseek-flash（V4.1 Flash，支持图像输入）。填写密钥并保存，可测试连接或读取模型。识图沿用图片确认流程，使用非思考模式；连接测试只检查文本请求，不代表图像识别质量。", "DeepSeek: select DeepSeek in AI settings. The default endpoint is https://api.deepseek.com/chat/completions and model is deepseek-flash (V4.1 Flash, with image input). Enter and save your key, then test the connection or load models. Recognition uses the existing image review flow in non-thinking mode. Connection tests check text requests, not image recognition quality."]
  ] }
];
