...
Card Vault v1.2.0 支持可选 Authenticode 签名。没有证书时仍可生成 Windows 安装包和便携包；配置签名凭据后，发布流程会自动签名 `Card Vault.exe` 和 NSIS 安装器，便携 ZIP 包含同一个已签名主程序。
...
没有有效证书不会阻止 v1.2.0 打包，但未签名文件可能触发 Windows 风险提示。开发调试继续使用 `npm run electron`，同样不要求发布证书。
...
```powershell
Get-AuthenticodeSignature -LiteralPath '.\dist\card-vault-1.2.0-setup.exe' |
